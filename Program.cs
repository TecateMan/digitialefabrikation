...
namespace TestApplication
{
    public static class Program
    {
        ...
        {
            var myBrick = new Brick<GyroSensor, Sensor, Sensor, Sensor>("usb");
            myBrick.Sensor1.Mode = GyroMode.Angle;
            
            myBrick.Sensor2 = new UltrasonicSensor();
           
            try
            {
               
                myBrick.Connection.Open();
            }
            catch (Exception x)
            {
            ...
            while (true)
            {
                try
                {
                    Console.WriteLine("MyGyro:  " + myBrick.Sensor1.ReadAsString());
                }
                catch (Exception ex)
                {
